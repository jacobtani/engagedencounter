...
class OrganiserMailer < ApplicationMailer
  ...
  def new_registration(registered_couple:)
    ...
    send_smtp_email = {
      ...
      'htmlContent'=> "<!DOCTYPE html><html><body><h1>New Registration on EE Website</h1>
      <p>Name: #{registered_couple.preferred_name}</p>
      <p>Age: #{registered_couple.age.to_s}</p>
      <p>Address: #{registered_couple.address}</p>
      <p>Email: #{registered_couple.email}</p>
      <p>Phone Number: #{registered_couple.phone_number}</p>
      <p>Religion: #{registered_couple.address}</p>
      <p>Parish: #{registered_couple.address}</p>
      <p>Fiance Name: #{registered_couple.fiance_preferred_name}</p>
      <p>Fiance Age: #{registered_couple.fiance_age.to_s}</p>
      <p>Fiance Address: #{registered_couple.fiance_address}</p>
      <p>Fiance Email: #{registered_couple.fiance_email}</p>
      <p>Fiance Phone Number: #{registered_couple.fiance_phone_number}</p>
      <p>Religion: #{registered_couple.fiance_religion}</p>
      <p>Parish: #{registered_couple.fiance_parish}</p>
      <p>Wedding Date: #{registered_couple.wedding_date.strftime('%-d %B %Y')}</p>
      <p>Preferred Event: #{registered_couple.address}</p>
      <p>Post Wedding Address: #{registered_couple.post_wedding_address}</p>
      <p>Dietary Requirements: #{registered_couple.dietary_requirements}</p>
      </body></html>",
      'sender'=> {
